...
public class NatureSimulator extends GridGame {
		...
		for (Creature creature : creaturesCopy) {
			...
			if (action != null) {
				if (action.getType() == Action.Type.STAY) {
					...
					addCreature(creature.reproduce(creaturesMap));

				} else if (action.getType() == Action.Type.MOVE) {
					// MOVE
					if (isDirectionFree(creature.getX(), creature.getY(), action.getDirection())) {
						creature.move(action.getDirection(), creaturesMap);
					}
					...
					if (attackedCreature != null) {
						creature.attack(attackedCreature, createLocalInformationForCreature(creature));
	...
	private LocalInformation createLocalInformationForCreature(Creature creature) {
		...
		Food food= new Food(0,0);  //to send the existing food as parameter 
		for(Creature c : this.creatures) {
			if(c instanceof Food) {
				food = new Food(c.getX(), c.getY());
			}
		}
		...
		return new LocalInformation(getGridWidth(), getGridHeight(), creatures, freeDirections, food);
	}
	...
	/**
	 * 
	 * @param x creature's x coordinate
	 * @param y creature's y coordinate
	 * @param creature 
	 * 
	 * if creature that is sent as paramaters is a snake, adds its all cell to the map.
	 */
	private void updateCreaturesMap(int x, int y, Creature creature) {
		if(creature instanceof Snake) {
			for(Cell c : ((Snake) creature).getHead()) {
				if (isPositionInsideGrid(c.getX(), c.getY())) {
					creaturesMap[c.getX()][c.getY()] = c;
				}
			}
		}
		else {
